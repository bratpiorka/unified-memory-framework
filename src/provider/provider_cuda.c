...
umf_result_t cu2umf_result(CUresult result) {
    switch (result) {
    case CUDA_SUCCESS:
...
static umf_result_t cu_memory_provider_alloc(void *provider, size_t size,
    ...
    // check the alignment
    ASSERT(alignment == 0 || ((uintptr_t)(*resultPtr) % alignment) == 0);
...
void cu_memory_provider_get_last_native_error(void *provider,
    ...
    size_t buf_size = 0;
    strncpy(TLS_last_native_error.msg_buff, error_name, TLS_MSG_BUF_LEN);
